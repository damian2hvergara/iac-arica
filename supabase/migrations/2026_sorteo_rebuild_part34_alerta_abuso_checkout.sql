-- ============================================================
-- IAC ARICA 2026 — Parte 34
-- Verificación en vivo del sistema de monitoreo mostró que la alerta
-- de "texto con forma de XSS/SQLi" (parte 32) se guarda en
-- system_events pero NUNCA manda correo: create_pending_order la
-- llama al toque desde el navegador del comprador, sin ningún Edge
-- Function de por medio, así que no hay dónde enganchar el aviso
-- inmediato en SQL puro (evitamos a propósito pg_net/Vault por
-- fragilidad, ver parte 31). Y como la compra igual se completa
-- (no se bloquea), el cliente tampoco ve un error que reportar.
--
-- Fix: create_pending_order ahora devuelve un campo extra
-- "abuso_detectado" — el cliente (stamper.html) lo revisa después de
-- una compra exitosa y, si viene en true, llama a report-client-error
-- (que sí dispara el correo vía el mismo camino ya probado). Se
-- mantiene además el registro directo en system_events desde SQL,
-- como respaldo por si el aviso del navegador no llega a salir.
-- ============================================================

DROP FUNCTION IF EXISTS create_pending_order(text, text, text, text, text, text);
